...
class DeepQAgent(object):
    ...
    def __init__(self, input_shape, nb_actions,
                 ...
                 memory_size=50000, train_after=1000, train_interval=4, target_update_interval=1000,
                 monitor=True):
...
def compute_reward(img, collision_info):

    if collision_info.has_collided:
        reward = -100
    else:
       img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
       roi = img_hsv[120:140, 55:230, :]
       g = [0, 0, 0] #L<C<R
       r = [0, 0, 0]
...
zc = client.getMultirotorState().kinematics_estimated.position.z_val - 15
...
while(flag):
        ...
        if((responses[0].height == 0) | (responses[0].width == 0)):
             flag = 1
             print('hi stuck')
        else:
             flag = 0
...
count = 0
overall_reward = []
overall_reward.append([])
while True:
    client.enableApiControl(True)
    client.armDisarm(True)
    action = agent.act(current_state)
    steering = interpret_action(action)
    print('steer',0.5*steering)
    p = client.getMultirotorState().kinematics_estimated.position
    x = p.x_val
    y = p.y_val
    z = p.z_val
    client.rotateToYawAsync(0.5*steering).join()
    client.moveToPositionAsync(x - 0.2, y - 0.3, zc, 0.3).join()
    print(x,y,z)
    
    f = 1
    while(f):
        image_response = client.simGetImages([airsim.ImageRequest("0", airsim.ImageType.Scene, False, False)])[0]
        if((image_response.height == 0) | (image_response.width == 0)):
                f = 1
        else:
                f = 0
    ...
    reward = compute_reward(image_rgb, collision_info)


    overall_reward[current_step].append(reward)
    count += 1
    ...
    if done:
        client.enableApiControl(True)
        client.armDisarm(True)
        score = math.sqrt(((x - xc)*(x - xc)) + ((y - yc)*(y - yc)))
        print('score : ',score)
        client.moveByVelocityAsync(0, -1, 0, 10).join()
        client.moveToPositionAsync(xc, yc , zc, 5).join()
        #client.moveToPositionAsync(, -31.9786, -19.0225, 2).join()
